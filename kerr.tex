...
\title{\textbf{Self-consistency of the Kerr principal null congruence:\\
...
% ─── Abstract ────────────────────────────────────────────
\begin{abstract}
We establish a self-consistency identity for the principal null
congruence of the Kerr metric and derive its equivalence with the
vacuum condition.
Setting $\sigma:=\sqrt{\Sigma}$ with $\Sigma:=r^2+a^2\cos^2\!\theta$,
and $\Box_{\mathrm{null}}\phi:=\partial_r^2\phi+(2r/\Sigma)\,\partial_r\phi$
along the principal outgoing null congruence, the functional
$K:=\Sigma\,\Box_{\mathrm{null}}\ln\sigma$ equals $1$ identically
for all $(r,\theta,M,a)$.
The proof is three lines; the key cancellation
$a^2\cos^2\!\theta-r^2+2r^2=\Sigma$ is the definition of~$\Sigma$.
Via the Raychaudhuri equation---with shear vanishing by the
Goldberg--Sachs theorem and the kinematic identity
$\tfrac{1}{2}\theta_{\mathrm{exp}}^2+\omega_{ab}\omega^{ab}=2/\Sigma$---the
condition $K{=}1$ is equivalent to
$R_{\mu\nu}\ell^\mu\ell^\nu=0$, characterising the vacuum condition
along the principal null direction.

As a spherically symmetric precursor, self-consistency functionals
$K_1:=r^2\Box\ln\sqrt{\Delta}$ and $K_2:=r^2\Box\ln r$,
with $\Box:=r^{-2}\partial_r(r^2 f\,\partial_r)$, satisfy
$K_1=1-\tfrac{1}{2}r^2R$,\;$K_2=1-8\pi r^2\rho$,\;
$K_1-K_2=8\pi r^2 p_{\!\perp}$ identically for any $C^2$ mass
function, encoding all independent components of the spherically
symmetric Einstein equations.
The $\theta$-dependence of $\sqrt{\Sigma}$ is essential:
no purely radial function satisfies the self-consistency condition.
\end{abstract}

\noindent\textbf{Keywords:} self-consistency $\cdot$ null congruence
$\cdot$ Kerr metric $\cdot$ exact identity
$\cdot$ Raychaudhuri equation $\cdot$ Newman--Penrose formalism

\medskip

What distinguishes $\sqrt{\Sigma}$ among all functions on the Kerr
spacetime?
It appears in the expansion scalar
$\theta_{\mathrm{exp}}=2r/\Sigma$ of the principal null congruence,
in every Christoffel symbol of Boyer--Lindquist coordinates, and
governs Carter separability of wave equations.
This note identifies a further role: $\sqrt{\Sigma}$ is the unique
function, up to a multiplicative constant, satisfying a
self-consistency condition with respect to the null wave operator
$\Box_{\mathrm{null}}$ defined by the congruence's own expansion.
The identity $\Sigma\,\Box_{\mathrm{null}}\ln\sqrt{\Sigma}=1$ holds
throughout the spacetime for all $(r,\theta,M,a)$; its geometric
content, established via the Raychaudhuri equation, is the
equivalence $\Sigma\,\Box_{\mathrm{null}}\ln\sqrt{\Sigma}=1
\Leftrightarrow R_{\mu\nu}\ell^\mu\ell^\nu=0$.
Section~\ref{sec:sph} establishes the spherically symmetric
precursor, Section~\ref{sec:kerr} proves the Kerr identity, and
Section~\ref{sec:discussion} derives the vacuum equivalence and
discusses uniqueness---offering a perspective internal to the
geometry rather than externally imposed.
...
\begin{equation}\label{eq:theta}
  ...
  = \frac{2r}{\Sigma}.
\end{equation}
We define the null d'Alembertian along this congruence:
...
%───────────────────────────────────────────────────────
\section{Spherically symmetric case}\label{sec:sph}
...
%───────────────────────────────────────────────────────
\section{Kerr extension via null geodesic congruence}\label{sec:kerr}
...
For the Schwarzschild case ($m=M$ constant), this recovers
$K_2=1$ in the spherically symmetric notation~\cite{K1}.
...
length scale of Kerr: it governs Carter separability and appears in
every Kerr Christoffel symbol.
...
($\hat\sigma_{ab}=0$).
Writing $\theta:=\theta_{\mathrm{exp}}=2r/\Sigma$ for the
expansion scalar, the Raychaudhuri
equation~\cite{Raychaudhuri1955} reduces to
...
from $\theta=2r/\Sigma$ and $\omega_{ab}\omega^{ab}=2a^2\cos^2\!\theta/\Sigma^2$;
the numerator $2r^2+2a^2\cos^2\!\theta=2\Sigma$ is the definition of $\Sigma$,
so this holds for the entire Kerr family regardless of $\Delta$):
...
$R_{\mu\nu}\ell^\mu\ell^\nu$ vanishes.
This is not a consequence of the Goldberg--Sachs theorem:
$K{=}1$ holds algebraically for all Kerr metrics regardless
of the matter content; the equivalence with
$R_{\mu\nu}\ell^\mu\ell^\nu=0$ is the new content of
...
$\partial_r\ln\sigma=r/\Sigma$, which uniquely (up to a
multiplicative constant) fixes $\sigma^2\propto\Sigma$.
Any other choice of $\sigma^2$ would leave a residual
proportional to the mismatch with $\Sigma$.
This is the precise sense of self-consistency: since
$\theta_{\mathrm{exp}}=2\,\partial_r\ln\sigma$, the operator
$\Box_{\mathrm{null}}$ is determined by $\sigma$ itself,
and $K{=}1$ is the condition that $\sigma$ satisfies its
own operator equation.
...
\end{equation}
Substituting $s=(r-M)/M$ shows this is an ODE with a
regular singular point at $r=2M$ whose indicial equation
has a double root at zero; by the Frobenius method, the
