...
and recovers $K_2=1$ in the spherically symmetric notation
— the original result of~\cite{K1}.
...
\begin{proof}
Since $D\ln\sigma=\theta_{\mathrm{exp}}/2$ (Section~\ref{sec:discussion},
\textit{Why~$\sqrt{\Sigma}$}), one has $D^2\ln\sigma=D\theta/2$, so
...
the Goldberg--Sachs theorem (Kerr is Petrov type~D), shear-free
($\hat\sigma_{ab}=0$).
The Raychaudhuri equation therefore reduces to
...
\noindent
Since Kerr is vacuum ($R_{\mu\nu}=0$), Proposition~\ref{prop:equiv}
is consistent with Theorem~\ref{thm:kerr}.
The full equivalence $K{=}1\Leftrightarrow R_{\mu\nu}=0$
requires one additional step: applying the result to both
principal null directions and invoking the polarisation
identity ($R_{\mu\nu}k^\mu k^\nu=0$ for all null $k^\mu$
plus $R=0$ implies $R_{\mu\nu}=0$).
This programme remains open for general metrics.
